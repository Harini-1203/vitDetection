...
def on_enter(e):
    e.widget['background'] = 'dodgerblue'
...
main = tk.Tk()
main.title("Vitamin deficiency") #designing main screen
...
def uploadDataset():
    ...
    log_text.insert(tk.END, "Dataset uploaded.\n")
...
def featuresExtraction():
    ...
    log_text.insert(END,"Total images found in dataset : "+str(X.shape[0])+"\n\n")
    log_text.insert(END,"Dataset train & test split. 80% dataset images used for training and 20% for testing\n\n")
    log_text.insert(END,"80% training images : "+str(X_train.shape[0])+"\n\n")
    log_text.insert(END,"20% testing images : "+str(X_test.shape[0])+"\n\n")
...
def calculateMetrics(algorithm, predict, y_test):
    ...
    recall.append(r)
    fscore.append(f)
    log_text.insert(END,algorithm+" Accuracy  :  "+str(a)+"\n")
    log_text.insert(END,algorithm+" Precision : "+str(p)+"\n")
    log_text.insert(END,algorithm+" Recall    : "+str(r)+"\n")
    log_text.insert(END,algorithm+" FScore    : "+str(f)+"\n\n")
...
def predict():
    ...
    cv2.putText(img, f'Vitamin deficiency predicted as: {weapon_label}', (10, 25),  cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
    cv2.imshow(f'Vitamin deficiency predicted: {weapon_label}', img)
    cv2.waitKey(0)

    # Display text alert
    log_text.insert(END, f'Weapon Predicted as: {weapon_label}\n')

    


bg_img = Image.open("image.jpg")
bg_img = bg_img.resize((1300, 700), Image.ANTIALIAS)
bg_img_tk = ImageTk.PhotoImage(bg_img)
bg_label = Label(main, image=bg_img_tk)
bg_label.place(x=0, y=0, relwidth=1, relheight=1)

font_title = ('times', 16, 'bold')
title = Label(main, text='Vitamin Deficiency using Image Processing with CNN', bg='white', fg='black', font=font_title)
title.config(height=3, width=120)
title.place(x=3, y=5)

font_text = ('times', 12, 'bold')
log_text = Text(main, height=15, width=150, font=font_text)
log_text.place(x=50, y=120)

scrollbar = Scrollbar(main, command=log_text.yview)
scrollbar.place(x=1200, y=120, height=310)
log_text.config(yscrollcommand=scrollbar.set)

# Style configuration for ttk buttons
style = ttk.Style()
style.configure('TButton', font=('times', 13, 'bold'), padding=10)
style.map('TButton',
          background=[('active', 'dodgerblue')],
          foreground=[('active', 'black')])

# Create ttk buttons
uploadButton = ttk.Button(main, text="Upload Kaggle Dataset", command=uploadDataset)
uploadButton.place(x=50, y=500)

featuresButton = ttk.Button(main, text="Data Preprocessing", command=featuresExtraction)
featuresButton.place(x=370, y=500)

svmButton = ttk.Button(main, text="Run Existing SVM Algorithm", command=runSVM)
svmButton.place(x=750, y=500)

cnnButton = ttk.Button(main, text="Run Proposed CNN Algorithm", command=runCNN)
cnnButton.place(x=50, y=570)

graphButton = ttk.Button(main, text="Comparison Graph", command=graph)
graphButton.place(x=370, y=570)

predictButton = ttk.Button(main, text="Vitamin Deficiency from Test Image", command=predict)
predictButton.place(x=750, y=570)
